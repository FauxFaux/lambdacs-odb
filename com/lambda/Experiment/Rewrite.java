...
/*
  Read in a file and construct frequency tables for word order. Then use those tables
...
public class Rewrite {
    ...
    public static String concat(ArrayList al, int n) {  // ({"a", "b", "c"}, 2) -> "b c"
        ...
        String s = "";                                  // Easier to debug than SBuffers.
        int size = al.size();
        if (size < n) n = size;
...
class PrefixEntry {                             // Continuations based on previous n words.
    ...
    Hashtable completions = new Hashtable();    // {"time" -> <WordEntry "time">, ...}
