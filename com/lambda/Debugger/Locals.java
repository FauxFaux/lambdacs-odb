...
public abstract class Locals  {
    ...
    static HashMap			varNameTable = new HashMap();	// "com.lambda.Debugger.Debugger.main:15" -> {"i", "j"...}
    static HashMap			varTypeTable = new HashMap();	// "com.lambda.Debugger.Debugger.main:15" -> {Demo, ShadowInt, ...}
    static HashMap			retTypeTable = new HashMap();	// "com.lambda.Debugger.Debugger.main:15" -> Demo
    static int 				nTripletons = 0, nSingletons = 0, nMultitons = 0, nHLists = 0, nHLEntries = 0, emptyHLs=0;
    static int				wastedTripletons = 0, wastedMultitons = 0, nLObjects=0;
    static int				nBindings=0;
    static String[] 			defaultNames = {"var0", "var1", "var2", "var3", "var4", "var5", "var6", "var7",//Only for MyVector?
    ...
    public final void setMethodID(String mid) {methodID = mid;}

    abstract public int getNLocals();

    public void setNLocals(int nLocals) {throw new DebuggerException("Cannot change nLocals if not LocalsMore");}
    ...
    static void appendVarTypes(String methodID, String[] sa, String returnTypeString, ClassLoader cl) {
	Class[] varTypes = new Class[sa.length];
	for (int i = 0; i < sa.length; i++) {
	    String s = sa[i];
	    varTypes[i] = parseType(s, cl);
	}
	varTypeTable.put(methodID, varTypes);
	retTypeTable.put(methodID, parseType(returnTypeString, cl));
    }
	...
	String s = s0.replace('/', '.');
	if (s.startsWith("L") && s.endsWith(";")) {s = s.substring(1, s.length()-1);}		// "Lcom.lambda.Foo;" -> "com.lambda.Foo"

	if (s == "I")		{type = (isArray2?int[][].class:(isArray?int[].class:int.class));}
	else if (s == "B")  	{type = (isArray2?byte[][].class:(isArray?byte[].class:byte.class));}
	else if (s == "Z")  	{type = (isArray2?boolean[][].class:(isArray?boolean[].class:boolean.class));}
	else if (s == "C")  	{type = (isArray2?char[][].class:(isArray?char[].class:char.class));}
	else if (s == "S")  	{type = (isArray2?short[][].class:(isArray?short[].class:short.class));}
	else if (s == "J")  	{type = (isArray2?long[][].class:(isArray?long[].class:long.class));}
	else if (s == "F")  	{type = (isArray2?float[][].class:(isArray?float[].class:float.class));}
	else if (s == "D")  	{type = (isArray2?double[][].class:(isArray?double[].class:double.class));}
	else if (s == "V")  	{type = ShadowVoid.class;}
	else
	    {
		if (isArray2) s = "[[L"+s+";";
		if (isArray) s = "[L"+s+";";			//System.out.println("CL "+ Debugger.classLoader);
		try {type = Class.forName(s, true, cl);}
		catch (Exception e) {Debugger.println("Locals.parseType() could not find "+s);}
	    }
	...
	for (int i = 0; i < len; i++) {
		...
		if (varIndex < 0) continue;		// args are bound w/o a time
    ...
    static public Locals createLocals(int time, TraceLine tl, String methodID, int nLocals) {

	nLocals = nLocals - tl.getArgCount();
	switch (nLocals) {
	case 0:		return new Locals0(time, tl, methodID);
	case 1:		return new Locals1(time, tl, methodID);
	case 2:		return new Locals2(time, tl, methodID);
	case 3:		return new Locals3(time, tl, methodID);
	case 4:		return new Locals4(time, tl, methodID);
	case 5:		return new Locals5(time, tl, methodID);
	case 6:		return new Locals6(time, tl, methodID);
	case 7:		return new Locals7(time, tl, methodID);
	case 8:		return new Locals8(time, tl, methodID);
	case 9:		return new Locals9(time, tl, methodID);
	case 10:	return new Locals10(time, tl, methodID);
	default:	return new LocalsMore(time, tl, methodID, nLocals);
	}
    }
	...
	if (varNames == null) varNames = defaultNames;
	if (varNames.length <= i) return "MISSING_VAR_NAME";
	return varNames[i];
    }
	...
	for (int i = 0; i < varNames.length; i++) if (varNames[i] == varName) return i;
	return -1;
    }
	...
	Class type = (Class) retTypeTable.get(methodID);
	return type;
    }

    public Class getVarType(int i) {
	Class[] varTypes = (Class[]) varTypeTable.get(methodID);
	if (varTypes == null) return null;
	if (varTypes.length <= i) return null;
	return varTypes[i];
    }
	...
	String s = "[";
	for (int i = 0; i < getNLocals(); i++) {s+=getVarName(i)+", ";}
	return("<Locals "+methodID+" "+s+"]>");		//+" "+trace.toString(10)+">");
    }
	...
	if (varIndex < nArgsTL) {
		...
		hl = new HistoryListTripleton(creationTime, o);		// Assume we're doing an add().
		hl.add(time, value);
		tl.putArg(varIndex, hl);
		return;
	    }
	    HistoryList hlNew = hl.add(time, value);
	    if (hlNew != null) tl.putArg(varIndex, hlNew);
 	    return;
	}
	...
	if (v instanceof HistoryList) {
	    hl = (HistoryList) v;
	    HistoryList hlNew = hl.add(time, value);
	    if (hlNew != null) putHL(varIndex, hlNew);
	    return;
	}

	int t = getTime(varIndex);
	if (t > -1) {
	    hl = new HistoryListTripleton(t, v);		// Assume we're doing an add().
	    hl.add(time, value);
	    putHL(varIndex, hl);
	}
	else {
	...
	if (o instanceof HistoryList) return (HistoryList) o;
	return null;
    }
	...
	if (varIndex < nArgsTL) {
	    Object o = tl.getArgActual(varIndex);
	    if (o instanceof HistoryList) return (HistoryList) o;
	    return new HistoryListSingleton(creationTime, o);		// Assume we're doing an ShadowLocals
	}
	...
	if (o instanceof HistoryList) return (HistoryList) o;
	int t = getTime(varIndex);
	if (t > -1) return new HistoryListSingleton(t, o);
	return null;
    }
	...
	int nArgsTL = tl.getArgCount();
	if (varIndex < nArgsTL) throw new DebuggerException("L unbound IMPOSSIBLE");
	varIndex -= nArgsTL;
