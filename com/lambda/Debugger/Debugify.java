...
public final class Debugify implements Constants {
	static String version = "14.Nov.04";
	...
	static ObjectType methodType =
		new ObjectType("de.fub.bytecode.classfile.Method");
	...
	static boolean NO_PUTFIELD = false,
		...
		NO_IASTORE = false,
		NO_RETURN = false,
		NO_RETURNVALUE = false,
		NO_INVOKEVIRTUAL = false,
		NO_ARGUMENTS = false,
		NO_ASTORE = false,
		NO_ATHROW = false,
		NO_CATCH = false,
		NO_INVOKESTATIC = false,
		NO_NEW = false,
		NO_AASTORE = false,
		NO_PREVIOUS = false,
		NO_LOCKS = false,
		NO_WAITS = false,
		DONT_REPLACE_VECTOR = false;
	...
	// java -DPUTFIELD -DISTORE -DIINC -DIASTORE -DRETURN -DRETURNVALUE -DINVOKEVIRTUAL -DASTORE -DATHROW -DCATCH -DARGUMENTS -DINVOKESTATIC Debugify
	//-DATHROW -DCATCH -DASTORE -DAASTORE -DIASTORE -DRETURN -DRETURNVALUE -DINVOKEVIRTUAL -DIINC -DISTORE -DPUTFIELD -DPUTSTATIC -DINVOKESTATIC -DARGUMENTS -DNEW
		...
		try {
			processFiles : for (int i = 0; i < args.length; i++) {
				if (args[i].endsWith(".class")) {
					...
					//		    if (dontProcessPackage(className)) {Debugger.print("-"); continue;}
					if (dontProcessMethod(dontInstrument,
						"*",
						className,
						true)) {
						System.out.print("-");
	...
	public static void initialize() {
				...
				NO_PUTSTATIC =
					NO_ISTORE =
						NO_IINC =
							NO_IASTORE =
								NO_RETURN =
									NO_RETURNVALUE = NO_INVOKEVIRTUAL = true;
			NO_ASTORE =
				NO_ATHROW =
					NO_CATCH =
						NO_INVOKESTATIC =
							NO_ARGUMENTS =
								NO_IASTORE = NO_NEW = NO_AASTORE = true;
		}
		...
		if (System.getProperty("PUTFIELD_ONLY") != null) {
			NO_ISTORE =
				NO_IINC =
					NO_IASTORE =
						NO_RETURN = NO_RETURNVALUE = NO_INVOKEVIRTUAL = true;
			NO_ASTORE =
				NO_ATHROW =
					NO_CATCH =
						NO_INVOKESTATIC = NO_ARGUMENTS = NO_PREVIOUS = true;
		}
	...
	public static JavaClass debugifyClass(
		...
		synchronized (D.class) {
			...
			//out = cpg.addFieldref("java.lang.System", "out", "Ljava/io/PrintStream;");
			//	    printlnString = cpg.addMethodref("java.io.PrintStream", "println", "(Ljava/lang/String;)V");
			toString =
				cpg.addMethodref(
					"java.lang.Object",
					"toString",
			...
			typeShadowBoolean =
				new ObjectType("com.lambda.Debugger.ShadowBoolean");
			...
			typeShadowDouble =
				new ObjectType("com.lambda.Debugger.ShadowDouble");
			...
			try {
				...
				for (int j = 0; j < fields.length; j++) {
					...
					if (!SILENT)
						Debugger.println(
							fields[j] + " Private: " + fields[j].isPrivate());
				}
				...
				for (int j = 0; j < methods.length; j++) {
					...
					if (name.equals("<clinit>")) {
						...
						classGen.replaceMethod(
							methods[j],
							debugifyMethod(classGen, methods[j]));
					}
				}
				if (clPosition > -1)
					classGen.replaceMethod(
						methods[clPosition],
						debugifyCLinit(classGen, methods[clPosition]));
	...
	public static JavaClass publicifyClass(
		JavaClass javaClass1,
		String classFileName) {
		synchronized (D.class) {
			ConstantPool constants = javaClass1.getConstantPool();
			...
			try {
				...
				Debugger.println(
					"FAILED Debugified: " + classFileName + " " + className);
				if (e instanceof DebuggerException)
					outputStream.println(e);
				else {
					if (Debugger.DEBUG_DEBUGGER)
	...
	static boolean dontProcessPackage(String cName) {
		...
		for (int i = 0; i < len; i++) {
			...
			//Debugger.println("only methods: " + cName + "."+  "? startswith " + iOnly);
			if (iOnly.equals("")) {
	...
	static boolean dontProcessMethod(
		VectorD cmPairs,
		String mName,
		String cName,
		boolean exact) { // true -> dont instrument/record
		...
		for (int i = 0; i < len; i++) {
			...
			if (exact) {
				...
				mName.equals("*")
					|| mName.equals(cmPair[1])
					|| cmPair[1].equals("*")
					|| (mName.equals("<init>")
						&& cmPair[1].equals("new"))) { // if mName is a candidate
				//Debugger.println("hidden methods: " + cName + "."+ mName + "?= " + cmPair[0] + "." + cmPair[1]);
				if (cmPair[0].equals("*"))
					return true;
				if (cName.equals("*"))
					return true;
	...
	static void createCLinitPatch(
		...
		patch.append(
			factory.createInvoke(
				...
				new Type[] { Type.INT },
				Constants.INVOKESTATIC));
		LocalVariableGen lvgArray =
			clinit.addLocalVariable("lvArray", stringArray, null, null);
		...
		patch.append(
			factory.createInvoke(
				...
				Constants.INVOKESTATIC));
		patch.append(
			factory.createPutStatic(className, "ODB_offset", Type.INT));
	}
	...
	static void createCLinit(ClassGen classGen) { // If there is no clint
		InstructionList patch = new MyInstructionList();
		MethodGen clinit =
			new MethodGen(Constants.ACC_STATIC, Type.VOID, new Type[] {
		}, new String[] {
		}, "<clinit>", className, patch, cpg);
	...
	private static Method debugifyCLinit(
		ClassGen classGen,
		Method m) { // If there IS a clinit
		InstructionList patch = new MyInstructionList();
	...
	private static Method debugifyMethod(ClassGen classGen, Method m) {
		...
		patch.append(
			factory.createGetStatic(className, "ODB_offset", Type.INT));
		patch.append(
			new PUSH(cpg, buildFileLineN(className, sourceFileName, line)));
...

		{
			...
			for (int i = 0; i < nArgsRecorded; i++) {
				...
				//if ((isStatic) && (lvs.length <= i))  outputStream.println("IMPOSSIBLE1");
				if (isStatic)
		...
		for (int j = 0; j < ihs.length; j++) {
			...
			// This is a messy way of ensuring that references to THIS are not touched
			// before the super constructor is called. An ugly compiler could fool us
			...
			if ((ins instanceof AASTORE)
				|| (ins instanceof IASTORE)
				|| (ins instanceof LASTORE)
				|| (ins instanceof FASTORE)
				|| (ins instanceof DASTORE)
				|| (ins instanceof SASTORE)
				|| (ins instanceof CASTORE)
				|| (ins instanceof BASTORE)) {
		...
		if (code1.getLength() > 60000)
			throw new DebuggerException(
				"Method too long. Instrumented version of "
					+ m
					+ " "
					+ code1.getLength()
					+ " > 60,000 bytes."
		...
		/*
		  if ((code1.getLength() > 9000) || ( ((code1.getLength()*10) / code.getLength()) > 50) ) {
		  Debugger.println("Orig Code length: "+code.getLength());
		  Debugger.println("Instrumented Code length: "+code1.getLength());
		  Debugger.println("Ratio: "+ ((code1.getLength()*10) / code.getLength()));
		  }
		*/
		return m;
	}
	...
	*/
	...
	static void doMethodLock(InstructionList patch) {
		...
		if (mg.isStatic() || name.equals("<init>")) {
			...
			//patch.append(factory.createInvoke("java.lang.Class", "forName", typeClass, new Type[] {Type.STRING}, Constants.INVOKESTATIC));
	...
	static void doMONITORENTER(int j) {
		...
		LocalVariableGen lg1 =
			mg.addLocalVariable("obj", Type.OBJECT, null, null);
		...
		//if (!SILENT) Debugger.println("Inserting throw:\n"+patch.toString(true));
	...
	static void doMethodLockExit() { // When you see a synchronized method
		...
		InstructionHandle exceptionIns =
			patch.append(
				new PUSH(cpg, buildFileLineN(className, sourceFileName, line)));
		patch.append(
			factory.createGetStatic(className, "ODB_offset", Type.INT));
	...
	static void doLockReturn(int j) { // when you see a return() instruction in a synch method
		if (NO_LOCKS)
			return;
	...
	static void doINVOKESPECIAL(int j) {
		...
		if (!SILENT)
			Debugger.println(
				"doINVOKESPECIAL " + methodClass + "." + methodName + "()");
		...
		if (methodClass.equals("java.util.Vector"))
			...
			patch.append(
				factory.createInvoke(
					"com.lambda.Debugger.MyHashtable",
					"<init>",
					Type.VOID,
					types,
					Constants.INVOKESPECIAL));
			...
			patch.append(
				factory.createInvoke(
					"com.lambda.Debugger.MyArrayList",
					"<init>",
					Type.VOID,
					types,
					Constants.INVOKESPECIAL));
		...
		*/
		...
		for (int i = start; i < argNames.length; i++) {
			...
			if (isStatic)
				...
				index = lvs[i + 1].getIndex();
			//      if (!SILENT) Debugger.println(mg.getName()+" " +argNames[i]+": " +argTypes[i]+" "+lvg);
	...
	//catchEx(String sourceFileName, int line, Object ex, TraceLine tl) {//Exception
	public static void doCATCH(Code code) {
		...
		if (!SILENT)
			Debugger.println(
				"==========code.getExceptionHandlers() START-------------");
		...
		loop : for (
			int i = 0;
				i < ceg.length;
				i++) { //Don't repeat if multiple exceptions have same handler
		...
		for (int i = 0; i < handlers.size(); i++) {
			...
			LocalVariableGen lg1 =
				mg.addLocalVariable("exc", Type.OBJECT, null, null);
			...
			//	    if (!SILENT) Debugger.println("Inserting catch:\n"+patch.toString(true));

			for (int ii = 0;
				ii < ceg.length;
				ii++) { // Retarget all exceptions using this hander
	...
	//    public static synchronized void throwEx(String sourceFileName, int line, Exception ex, TraceLine tl) {
	...
	public static void createArray1DPatch(int j) {
		...
		// Change instance var: D.changeArray1D("foo.java", 75, Int[23]{3, 4, ...}, index, <Obj_33>, tl) SET INSTANCE VAR
		patch.append(
			factory.createGetStatic(className, "ODB_offset", Type.INT));
		patch.append(
			new PUSH(cpg, buildFileLineN(className, sourceFileName, line)));
		...
		//if (!SILENT) Debugger.println("Inserting (D_changeArray1D:\n"+patch.toString(true));
		//il.insert(ihs[j], patch);
	...
	public static void createChangeIVPatch(boolean isStatic) {
		...
		LocalVariableGen lg2 =
			mg.addLocalVariable("value", fieldType, null, null);
	...
	/*
	  ...
	  Type fieldType = ((FieldInstruction)ins).getFieldType(cpg);
	  Type classType = ((FieldInstruction)ins).getClassType(cpg);
	  LocalVariableGen lg2 = mg.addLocalVariable("value", fieldType, null, null);
	  int value = lg2.getIndex();
	  Instruction storeIns, loadIns1, loadIns2;
	  InvokeInstruction shadowIns;
	  int obj= -1;
	  LocalVariableGen lg1=null;
	
	
	  if ((fieldType == Type.INT) || (fieldType == Type.SHORT) || (fieldType == Type.BYTE) ||
	  (fieldType == Type.CHAR) || (fieldType == Type.BOOLEAN)) {
	  storeIns = new ISTORE(value);
	  loadIns1 = new ILOAD(value);
	  loadIns2 = new ILOAD(value);
	  if (fieldType == Type.INT) shadowIns = new INVOKESTATIC(D_createShadowInt);
	  else if (fieldType == Type.SHORT)  shadowIns = new INVOKESTATIC(D_createShadowShort);
	  else if (fieldType == Type.BYTE)  shadowIns = new INVOKESTATIC(D_createShadowByte);
	  else if (fieldType == Type.CHAR)  shadowIns = new INVOKESTATIC(D_createShadowChar);
	  else if (fieldType == Type.BOOLEAN) shadowIns = new INVOKESTATIC(D_createShadowBoolean);
	  else return; //impossible
	  if (!SILENT) Debugger.println("doPUTFIELD " + fieldName + " " + fieldType + " " + shadowIns.getMethodName(cpg));
	  ...
	  storeIns = new LSTORE(value);
	  loadIns1 = new LLOAD(value);
	  loadIns2 = new LLOAD(value);
	  shadowIns = new INVOKESTATIC(D_createShadowLong);
	  }
	  else if (fieldType == Type.DOUBLE) {
	  storeIns = new DSTORE(value);
	  loadIns1 = new DLOAD(value);
	  loadIns2 = new DLOAD(value);
	  shadowIns = new INVOKESTATIC(D_createShadowDouble);
	  }
	  else if (fieldType == Type.FLOAT) {
	  storeIns = new FSTORE(value);
	  loadIns1 = new FLOAD(value);
	  loadIns2 = new FLOAD(value);
	  shadowIns = new INVOKESTATIC(D_createShadowFloat);
	  }
	  else 
	  return;
	  ...
	  if (!isStatic) {
	  lg1 = mg.addLocalVariable("obj", Type.OBJECT, null, null);
	  obj = lg1.getIndex();
	  lg1.setStart(patch.append(new ASTORE(obj)));				// This is the object for 'this'
	  patch.append(new ALOAD(obj));				// Load the 'this' for him
	  }
	  patch.append(loadIns1);				// Load the value for him
	  // Now the stack is restored.
	
	  // Change instance var: D.change("foo.java", 75, <Obj_12>, "a", <Obj_33>, tl) SET INSTANCE VAR
	  ...
	  //patch.append(factory.createInvoke("java.lang.Class", "forName", typeClass, new Type[] {Type.STRING}, Constants.INVOKESTATIC));
	  //patch.append(new INVOKESTATIC(D_createShadowClass1));
	  patch.append(getClassFromName(classType.toString()));
	
	  }
	  else 	
	  lg1.setEnd(patch.append(new ALOAD(obj)));		// Load the 'this' for us
	  patch.append(new PUSH(cpg, fieldName));				
	  lg2.setEnd(patch.append(loadIns2));		// Load the value for us
	  if (shadowIns != null) patch.append(shadowIns);
	  patch.append(new ALOAD(tl));
	...
	*/
	...
	public static void checkForRET() { // List ret targets in array & DON'T ASTORE them
		retIndex = 0;
		for (int j = 0; j < ihs.length; j++) {
			Instruction ins = ihs[j].getInstruction();
			if (!(ins instanceof RET))
				continue;
			//	  system.out.println("There's a RET!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");// XXXXXXXXXXXXXX
			RET retIns = (RET) ins;
	...
	public static void doRETURN(int j) { // Just a marker -- robust
		...
		if (returnType == Type.LONG) {
			...
			if (returnType == Type.INT)
				...
				Debugger.println(
					"IMPOSSIBLE: Appending returnValue " + returnType);
				return;
			}
		}
		patch.append(
	...
	public static void doINVOKEVIRTUAL(
		int j,
		boolean isStatic,
		boolean isSpecial) {
		...
		if ((methodName.equals("arraycopy"))
			...
			patch.append(
				factory.createInvoke(
					"com.lambda.Debugger.MyArrays",
					"sort",
					Type.VOID,
					types,
					Constants.INVOKESTATIC));
		else if (
			(methodName.equals("fill"))
				&& (methodClass.equals("java.util.Arrays")))
			patch.append(
				factory.createInvoke(
					"com.lambda.Debugger.MyArrays",
	...
	public static void doINVOKEVIRTUAL1(
		int j,
		boolean isStatic,
		boolean isSpecial) {
		if (NO_INVOKEVIRTUAL)
		...
		//OneObj = (!dontRecord.contains(methodName));    // && (argumentTypes.length <= MAX_ARGS_RECORDED));
		if (!OneObj)
			return;
		...
		for (int i = argumentTypes.length - 1;
			i >= 0;
			i--) { // Store the args for us
		...
		// ************************NOW START LOADING THE ARGUMENTS FOR INVOKE()*************************
		// We will only load a max of MAX_ARGS_RECORDED arguments.
		// THIS WILL BE A LITTLE TRICKY... LOAD THE FIRST MAX_ARGS_RECORDED ARGUMENTS
		//  public static synchronized TraceLine invoke(String sourceFileName, int line, Object o, String meth)
		...
		patch.append(new IADD());
		if (isStatic || isNew) {
			//if (isSpecial && !isNew) insertGetClass("com.lambda.Debugger.SUPER", patch);			else
		...
		//    if (methodName.equals("join") && methodClass.equals("java.lang.Thread")) {
		...
		// After the INVOKEVIRTUAL returns, we'll record the value and insert a D.returnValue()
		// The stack is CLEAN when we start & finish this. Unrelated to the above.
	...
	/*
	  ...
	  LocalVariableGen lg1 = mg.addLocalVariable("value", fieldType, null, null);
	  int value = lg1.getIndex();
	  Instruction storeIns, loadIns1, loadIns2;
	  InvokeInstruction shadowIns;
	  ...
	  patch.append(loadIns1);
	  if (shadowIns != null) patch.append(shadowIns);
	  patch.append(new ALOAD(tl2));
	  patch.append(new INVOKESTATIC(D_returnValue_1));
	  lg1.setEnd(patch.append(loadIns2));
	...
	*/
	...
	public static void appendPatch(InstructionHandle ih, String debug) {
		...
		/* NOT NECESSARY? BECAUSE WE PROMISE NEVER TO THROW FROM OUR CODE: D_invoke() etc.
		   for (int i = 0; i < ceg.length; i++) {			// The end of handlers for monitors must cover our code.
		   InstructionHandle handlerEndPC= ceg[i].getEndPC();	// The other handlers, doesn't matter.
		   if (ih == handlerEndPC) {
		   if (!SILENT) Debugger.println("appendPatch redirecting handler end from:" +ih + " to: "+ lastIHInPatch);
		   ceg[i].setEndPC(lastIHInPatch);
		   return;
		   }
		   }
		*/
	...
	public static void insertPatch(InstructionHandle ih, String debug) {
		...
		if (it != null) {
			for (int i = 0; i < it.length; i++) {
				if (it[i] instanceof CodeExceptionGen) {
					//if (!SILENT) Debugger.println("NOT Retargeting: " + it[i] + " from " + ih + " to " + firstIHInPatch); WHY NOT?
				} else {
					//if (!SILENT) Debugger.println("Retargeting: " + it[i] + " from " + ih + " to " + firstIHInPatch);
					it[i].updateTarget(ih, firstIHInPatch);
				}
			}
		}
	...
	static LocalVariable[] createMissingVarTable(LocalVariableGen[] lvs) {
		...
		for (int i = 0; i < lvs.length; i++) {
			...
			//	    if (lvg.getName().equals("this")) continue;   I HAVE TO LOOK LIKE THEM!
			v.add(lvg);
		}
		...
		for (int i = 0; i < newLV.length; i++) {
			newLV[i] =
				((LocalVariableGen) v.elementAt(i)).getLocalVariable(cpg);
	...
	static void appendVarMapping(String methodID, MethodGen m) {
		...
		patchVM.append(
			(CPInstruction) factory.createNewArray(Type.STRING, (short) 1));
		LocalVariableGen lvgArray =
			D_ODB_declareVarMappingsMethod.addLocalVariable(
				"lvArray",
				stringArray,
				null,
				null);
		...
		patchVM.append(
			factory.createInvoke(
				...
				"appendVarNames",
				Type.VOID,
				new Type[] { Type.STRING, stringArray },
				Constants.INVOKESTATIC));
	...
	static void createVarMappingsStart(ClassGen javaClass) {
		...
		D_ODB_declareVarMappingsMethod =
			new MethodGen(
				Constants.ACC_STATIC | Constants.ACC_PUBLIC,
				Type.VOID,
				new Type[] {
		}, new String[] {
		}, "ODB_declareVarMappings", className, patchVM, cpg);
	}

	static void createVarMappingsEnd(ClassGen javaClass) {
		if (patchVM.size() > 40000)
			throw new DebuggerException(
				"Too many variables in class "
	...
	static void bindMethodNames(InstructionList patch, ConstantPoolGen cpg) {
		...
		MethodGen D_getPreviousTLMethod =
			new MethodGen(
				Constants.ACC_STATIC
		...
		MethodGen D_addUnparentedMethod0 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod8 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod7 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod6 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod5 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod4 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod3 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod2 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod1 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod9 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_addUnparentedMethod10 =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeZMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.BOOLEAN,
				new Type[] { Type.BOOLEAN, Type.INT, Type.INT, traceLine },
				new String[] { "value", "slIndex", "varIndex", "tl" },
				"changeZ",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_changeSMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.SHORT,
				new Type[] { Type.SHORT, Type.INT, Type.INT, traceLine },
				new String[] { "value", "slIndex", "varIndex", "tl" },
				"changeS",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_changeFMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.FLOAT,
				new Type[] { Type.FLOAT, Type.INT, Type.INT, traceLine },
				new String[] { "value", "slIndex", "varIndex", "tl" },
				"changeF",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_changeDMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.DOUBLE,
				new Type[] { Type.DOUBLE, Type.INT, Type.INT, traceLine },
				new String[] { "value", "slIndex", "varIndex", "tl" },
				"changeD",
				"com.lambda.Debugger.D",
				patch,
		...
		D_bindMethod =
			new MethodGen(
				...
				new String[] { "slIndex", "varIndex", "value", "tl" },
				"bind",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		...
		// ChangeD instance var: D.changeD("foo.java", 75, <Obj_12>, "a", <Obj_33>, tl) SET INSTANCE VAR
		D_changeIVMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
				...
				new Type[] {
					...
					Type.OBJECT,
					traceLine },
				new String[] { "slIndex", "o", "varName", "value", "tl" },
				"change",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		...
		MethodGen D_changeIVAMethod =
			new MethodGen(
				...
				new Type[] {
					...
					Type.STRING,
					traceLine },
				new String[] { "o", "value", "slIndex", "varName", "tl" },
				"changeIVA",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		D_changeIVA = cpg.addMethodref(D_changeIVAMethod);
		MethodGen D_changeIVBMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayAMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				...
				new Type[] {
					...
					Type.INT,
					traceLine },
				new String[] { "array", "index", "value", "slIndex", "tl" },
				"changeArrayA",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		D_changeArrayA = cpg.addMethodref(D_changeArrayAMethod);
		MethodGen D_changeArrayZMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayBMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayCMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArraySMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayIMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayLMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayFMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_changeArrayDMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
	...
	static void bindMethodNames1(InstructionList patch, ConstantPoolGen cpg) {
		...
		MethodGen D_returnValueAMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		// XXX
		MethodGen D_returnValueBMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_returnValueCMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_returnValueSMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.SHORT,
				new Type[] { Type.SHORT, traceLine },
				new String[] { "rv", "tl" },
				"returnValueS",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_returnValueIMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_returnValueZMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.BOOLEAN,
				new Type[] { Type.BOOLEAN, traceLine },
				new String[] { "rv", "tl" },
				"returnValueZ",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_returnValueLMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
		...
		MethodGen D_returnValueFMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.FLOAT,
				new Type[] { Type.FLOAT, traceLine },
				new String[] { "rv", "tl" },
				"returnValueF",
				"com.lambda.Debugger.D",
				patch,
		...
		MethodGen D_returnValueDMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.DOUBLE,
				new Type[] { Type.DOUBLE, traceLine },
				new String[] { "rv", "tl" },
				"returnValueD",
				"com.lambda.Debugger.D",
				patch,
		...
		//	public static synchronized void throwEx(String slIndex, int line, Object ex, TraceLine tl)
		D_athrowMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.VOID,
				new Type[] { Type.INT, Type.OBJECT, traceLine },
				new String[] { "slIndex", "ex", "tl" },
				"throwEx",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		...
		//    public static synchronized void catchEx(String slIndex, int line, Object ex, TraceLine tl) 
		D_catchMethod =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
		...
		D_exitMethod =
			new MethodGen(
				...
				new Type[] {
					...
					traceLine,
					Type.OBJECT },
				new String[] { "slIndex", "o", "meth", "tl", "arg1" },
				"exit",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		...
		D_newObj_2Method =
			new MethodGen(
				...
				new Type[] {
					...
					Type.OBJECT,
					Type.OBJECT },
				new String[] { "slIndex", "o", "tl", "arg1", "arg2" },
				"newObj",
				"com.lambda.Debugger.D",
				patch,
				cpg);
		...
		D_returnMarker_1Method =
			new MethodGen(
				Constants.ACC_STATIC
					| Constants.ACC_PUBLIC
					| Constants.ACC_SYNCHRONIZED,
				Type.VOID,
				new Type[] { Type.OBJECT, Type.INT, traceLine },
				new String[] { "rv", "slIndex", "tl" },
		// INVERTED ARGS
	...
	// AspectJ: "Display1.java;spacewar/Debug.java[1k];coordination/Coordinator.java[2k];spacewar/RegistrySynchronization.java[3k]"
	// Java:    "Display.java"
	...
	static String buildFileLine0 (		String className,		String sourceFileName,		int line) {
		...
		if (line < 0) {
			if (!SILENT)
				Debugger.println(
					"Bad line number: " + sourceFileName + ":" + line);
		...
		for (int i = 0; i < v.size(); i++) {
			...
			if (fn0.endsWith("]")) {
				...
				if (startString.endsWith("k"))
					start0 =
						Integer.parseInt(
							startString.substring(0, startString.length() - 1))
							* 1000;
				// start = 2000
				else
					//throw new DebuggerException("Unknown start line "+startString+" in " +sourceFileName);
			...
			if (line >= start0) {
				...
				//Debugger.println("AspectJ: " + sourceFileName + ":"+line+" -> " +fn1+":"+line0);
	...
	private static int getLineNumber(int pc) {
		...
		try {
			...
			//	    Debugger.println("Bad pos in lineNumberTable for: "+pc+" instruction: " + ihs[pc]);	// This sucks! Fix BCEL
			line = -1;
		}
		return line;
	}
	...
	private static void insertGetClass(
		String className1,
		InstructionList patch) {
		IFNONNULL branch2;
	...
	static void createClassNameMethod(ClassGen classGen) {
		InstructionFactory factory = new InstructionFactory(cpg);
		InstructionList patch = new MyInstructionList();
		MethodGen D_ODB_classNameMethod =
			new MethodGen(
				Constants.ACC_STATIC | Constants.ACC_PUBLIC,
				Type.VOID,
				new Type[] {
		}, new String[] {
		}, "ODB_classNameMethod", className, patch, cpg);
		...
		while (it.hasNext()) {
			...
			patch.append(
				factory.createInvoke(
					...
					Constants.INVOKESTATIC));
			patch.append(
				factory.createPutStatic(className, varName, typeClass));
			*/
		}
		patch.append(InstructionConstants.RETURN);
...
class MyInstructionList extends InstructionList {
	...
	public InstructionHandle append(Instruction arg0) {
		if (!Debugify.SILENT) Debugger.println("\t"+arg0);
