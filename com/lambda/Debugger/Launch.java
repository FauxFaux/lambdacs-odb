...
public class Launch extends JFrame {
	...
	/*   CANNOT GET JAVAX.swing SOURCE  
	JButton openButton4 = new JButton("Test");
	openButton4.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent evt) {
		    Debugger.INSTRUMENT = false;
		    Debugger.DEMO = true;
		    Debugger.clazz = com.lambda.Debugger.Test.class;
		    Debugger.programName = "com.lambda.Debugger.Test";
		    StopButton.create(true, false, true, false);
		    setVisible(false);
		    launchWindow.dispose();
		    Debugger.startTarget(Debugger.clazz);	// Not called from cmd line. Cannot have args.
		}});
	panel.add(openButton4);
	*/
    ...
    public void launch(URL[] classPath, String className) throws Exception {
	...
	if (launchWindow.fileChooser.showOpenDialog(launchWindow) == JFileChooser.APPROVE_OPTION) {
	    try {
		launchWindow.launch(launchWindow.fileChooser.getSelectedFile());
	    } catch (Exception ex) {
		ex.printStackTrace();
	    }
	}
