...
public class DebugifyingClassLoader extends java.lang.ClassLoader {
    ...
    private static void println(String s) {out.println(s);}

    private static String spaces(int i) {
	if (i < 0) return spaces[0];
	if (i < 10) return spaces[i];
	return spaces[10];
    }
	...
	for (int i = 0; i < dontInstrument.size(); i ++) {
	    if (className.startsWith((String)dontInstrument.elementAt(i))) return true;
	}
	return false;
    }
	...
	if ((!className.startsWith("javax.xml.")) && (dontInstrument(className) || (!Debugger.INSTRUMENT) ) ) {
	    clazz = getParent().loadClass(className);
	    if (Debugger.TRACE_LOADER) println("loaded via parent: "+ getParent()+" " + className);
	    return clazz;
	}

	if (Debugger.TRACE_LOADER) println(spacesPlus()+"DebugifyingClassLoader loading:     " + className);
	try {clazz = findClass(className, instrument);}
	catch (VerifyError ve) {
	    println(spacesMinus()+"The ODB cannot instrument: " + className + ". Please report bug.\n"+ve);
	    if (Debugger.TRACE_LOADER) ve.printStackTrace();
	    clazz = getParent().loadClass(className);		
	}

	if (clazz==null) throw new java.lang.ClassNotFoundException(className);		//return null;
	if (resolve) resolveClass(clazz);
	return clazz;
    }

    public static byte[] debugify(String className, byte[] bytes) {
	if (!Debugger.INSTRUMENT) return bytes;
		if (Debugger.TRACE_LOADER) println("debugifying " + className);

        ClassParser parser = new ClassParser(new ByteArrayInputStream(bytes), "<generated>");
	JavaClass 	javaClass;
	try { javaClass = parser.parse(); }
	catch (IOException e) {
	...
	if (Debugger.TRACE_LOADER) println(spacesExact()+"DebugifyingClassLoader debugifying: " + className);
	if (Debugger.TRACE_LOADER_STACK) (new Exception("Just used to get stack trace")).printStackTrace();
	...
	if (Debugger.TRACE_LOADER) println(spacesMinus()+"DebugifyingClassLoader loaded:      " + className);
	return clazz;
    }
