...
public class CatchLine extends MethodLine {
	...
	if (throwLine != null)
	    if (throwLine.time <= divider) throwLine = null;
	if (caller != null)
	    if (caller.time <= divider) caller = null;
	...
	if (
	    ...
	    return;
	throw new DebuggerException("CL.verify() failed on time:"+time+" <CL "+this+"> "+TimeStamp.getTypeString(time));
    }


    public String toString() {
	if (printString == null)
	    printString = spaces((traceLine.getDepth()*2))+"Catch -> "+trimToLength(exception.value, 30);
	return printString;
    }
  ...
  public Object getSelectedObject(int x, FontMetrics fm) {
    ...
    if (x < fm.stringWidth(str)) return(exception.value);
    return(null);
  }
	...
	if (room < 50)
	    return ("<CatchLine " + time +" "+trimToLength(exception.value, 30)+">");
	if (room < 100)
	    return("<CatchLine " + time + " " + getSourceLine() +" "+getThread()+" "+" "+trimToLength(exception.value, 30)+" "+traceLine.toString(40)+">");
	return("<CatchLine " + time + " " + getSourceLine() +" "+getThread()+" "+" "+trimToLength(exception.value, 30)+" "+traceLine+" "+throwLine+">");
    }
	...
	VectorD traces = TraceLine.unfilteredTraceSets[TimeStamp.getThreadIndex(thread)];
	for (int i = traces.size()-1; i > -1; i--) {
		...
		if (tl.returnLine == null) {
		    ...
		    cl.caller = tl;			// The last is the one in Catch's stack frame.
		}
...
/*
	...
	CatchLine ca;
	ThrowLine th;
	TraceLine.addTrace(file, 2, obj , "init", argsList);
	aa = TraceLine.addTrace(file, 2, obj , "frob", argsList);
	ab = TraceLine.addTrace(file, 2, obj , "frob", argsList);
	//	ab.addLocal("i");
	//	ab.addLocal("j");
	//	ab.addLocal("k");
	r1 = ReturnLine.addReturnLine("Falke.t:1", 0, 6, "frob", "<Obj RETURN0>", ab);
	ba = TraceLine.addTrace("Falke.t:1", obj , "frob1", argsList);
	bb = TraceLine.addTrace("Falke.t:1", obj , "frob1", argsList);
	bc = TraceLine.addTrace("Falke.t:1", obj , "frob1", argsList);
	th = ThrowLine.addThrowLine("Falke.t:1", ex, bc);
	ca = CatchLine.addCatchLine("Falke.t:1", ex, aa);
	//r2 = ReturnLine.addReturnLine("Falke.t:1", 0, 10, "frob1", "<Obj RETURN1>", bc);  // THROWN OVER
	//r3 = ReturnLine.addReturnLine("Falke.t:1", 0, 12, "frob1", "<Obj RETURN2>", bb);
	//r4 = ReturnLine.addReturnLine("Falke.t:1", 0, 13, "frob1", "<Obj RETURN3>", ba);
	r5 = ReturnLine.addReturnLine("Falke.t:1", 0, 14, "frob", "<Obj RETURN4>", aa);	// HAND EDITED TIMES
