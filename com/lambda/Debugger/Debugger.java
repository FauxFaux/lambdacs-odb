...
public class Debugger extends JFrame {
	static String version = "20.Nov.04";
	...
	static boolean USE_BOOTCLASSLOADER =
		(InstrumentorForCL.class.getClassLoader() == null);
	static int MAX_MEMORY = 80000000;
	...
	static private void readCommandLineFlags() {
		...
		//String cd = System.getProperty("CUTOFF_DEPTH"); 	if (cd != null)  D.CUTOFF_DEPTH = Integer.parseInt(cd);
		String s = System.getProperty("MEMORY");
		if (s != null) {
			MAX_MEMORY = Integer.parseInt(s);
			TimeStamp.setMax(MAX_MEMORY / 200);
		}
		...
		//	if (System.getProperty("OUTPUT_ONLY") != null)		{OUTPUT_ONLY = true; PAUSED=true;}
		...
		//	    Debugger.println("Only calls less than "+D.CUTOFF_DEPTH+" deep will be recorded (and no assignments).");
		if (PAUSED)
			Debugger.println(
				"Recording will not start until 'Start Recording' is pushed");
		if (!INSTRUMENT)
	...
	private void initialize() {
		...
		ActionListener listener =
			new DebuggerActionListener(b3, b4, b5, b6, b7);
	...
	public static void revert(TimeStamp ts, boolean updateMessage) {
		...
		long clock =
			Clock.findTime(ts.time) - Clock.findTime(previousTime.time);
		if (updateMessage)
			Debugger.message(
				"From last: "
					+ (ts.time - previousTime.time)
					+ " stamps, "
		...
		updateTSLabel(ts);
		//	if (timeSlider.getValue() != ts.time) {timeSlider.setValue(ts.time); timeSlider.updateUI();}
		reverting = false;
	}
	...
	public static void updateThreadPanel(TimeStamp ts) {
		...
		if ((tp == null)
			|| (tp.tid != ts.getThread())) { // Only null once? Best default??
	...
	public static void updateTracePane(TimeStamp ts) {
		...
		//if ((ml instanceof ReturnLine) && (ml.filteredIndex == -1)) ml = ((ReturnLine) ml).traceLine; NEVER HAPPEN
		if (ml == null) {
			ml = TraceLine.defaultTraceLine();
		} // Just don't crash.
		//D.println("TraceLineB: "+ml.toString(0));
		if (previousTime.getThread() != ts.getThread()) {
			//D.println("Changing from "+ TimeStamp.previousTime().getThread() +" to "+ ts.getThread());
			TracePList.updateUI();
		}
		if (firstTimeTracePane) {
			firstTimeTracePane = false;
			TracePList.updateUI();
			//Where should I do this? When changing threads (above) and initializing (somewhere)
		}
	...
	public JPanel createThreadsPanel() {
		...
		JScrollPane sp =
			createJL(ThreadPane.singleton(), new ThreadListener(), true);
	...
	public JPanel createLocalsPanel() {
		...
		JScrollPane sp =
			createJL(LocalsPane.singleton(), new LocalsActionListener(), true);
		panel.add(sp);
	...
	public JPanel createThisPanel() {
		...
		JScrollPane sp =
			createJL(ThisPane.singleton(), new ThisActionListener(), true);
		panel.add(sp);
	...
	public static void updateLocalsPanel(TraceLine tl) {
		...
		if (previousLocals == locals) {
			...
			/*
			  ...
			  LocalsPane.singleton().displayList = v;                                            // Change soon!
			  int len = locals.locals.length;
			  for (int i=0; i < len; i++) v.add(locals.locals[i]);
			*/
	...
	public JPanel createTracePanel() {
		...
		JScrollPane sp =
			createJL(TraceLine.SINGLETON, new TraceListener(), true);
	...
	public JPanel createCodePanel() {
		...
		ActionListener listener =
			new CodeActionListener(b0, b1, b2, b3, b4, b5, b6, b7, b8, b9);
		b0.addActionListener(listener);
		b1.addActionListener(listener);
		b2.addActionListener(listener);
		b3.addActionListener(listener);
	...
	public static void main2(String args[]) {
		...
		if (CMD_LINE) { // if on command line w/a class name to run.
			...
			if (START) {
				//			new Thread(new Runnable() {public void run() {runMain(clazz2, argList2);}}, "Main").start();
	...
	static public Object runAlternate(
		Object obj,
		Method method,
		Object[] args) {
		...
		if (TimeStamp.eott() < 2) {
			if (!PAUSED)
				Debugger.println(
					"collected no data on this run.  Is target debugified?");
			return returnValue;
		}
	...
	static void runTarget(Class c, Object[] a) {
		...
		try {
			...
			System.out.println(
				"main() not static in target program?: \n"
					+ clazz
					+ "."
					+ method.getName()
					+ " threw "
	...
	public static void printSomeStatistics() {
		...
		System.out.println(
			" Realtime: "
				...
				+ " ("
				+ (TimeStamp.nTSCreated - TimeStamp.index)
				+ " GC'd, leaving "
				+ TimeStamp.index
				+ ") ");
		System.out.println(
			" Memory: "
				+ run.freeMemory() / 1000000
				+ "MB free / "
				+ run.totalMemory() / 1000000
				+ "MB max");
	}
	...
	public static void printAllStatistics() {
		...
		System.out.println(
			" HistoryList Entries: "
				+ HistoryList.nEntries
				+ "\n SourceLines: "
				+ SourceLine.nEntries
				+ " TraceLines: "
				+ TraceLine.nTraceLines
				...
				+ HistoryListSingleton.getNUpgraded()
				+ " Lookups: "
				...
				+ Shadow.nEntries()
				+ "\n Context Switches: "
				+ TimeStamp.nContextSwitches()
				+ "\n Time Debugifying Classes: "
				+ timeDebugifying / 1000.0
				+ " secs");
	...
	private void createFilterMenu(JMenuBar panel) {
		...
		JMenuItem filterMethodC =
			createJMenuItemAlt(
				menu,
				"Filter out method in class",
				KeyEvent.VK_F,
				null,
				listener);
		JMenuItem filterMethod =
			createJMenuItemAlt(
				menu,
				"Filter out method",
				KeyEvent.VK_M,
		...
		JMenuItem filterMethodI =
			createJMenuItemAlt(
				...
				listener);
		JMenuItem unfilter =
			createJMenuItemAlt(menu, "Unfilter", KeyEvent.VK_U, null, listener);
		JMenuItem filterClass =
			createJMenuItemAlt(
				menu,
				"Filter out all methods in class",
				0,
				null,
				listener);
		JMenuItem save =
		...
		JMenuItem filterIn =
			createJMenuItemAlt(
				...
				listener);
		JMenuItem filter1 =
			createJMenuItemAlt(menu, "Filter out depth>1", 0, null, listener);
		JMenuItem filter2 =
			createJMenuItemAlt(menu, "Filter out depth>2", 0, null, listener);
		JMenuItem filter3 =
			createJMenuItemAlt(menu, "Filter out depth>3", 0, null, listener);
		JMenuItem filter4 =
			createJMenuItemAlt(menu, "Filter out depth>4", 0, null, listener);
		JMenuItem filter5 =
			createJMenuItemAlt(menu, "Filter out depth>5", 0, null, listener);
		JMenuItem filter6 =
			createJMenuItemAlt(menu, "Filter out depth>6", 0, null, listener);
		JMenuItem filter7 =
			createJMenuItemAlt(menu, "Filter out depth>7", 0, null, listener);
		JMenuItem filter8 =
			createJMenuItemAlt(menu, "Filter out depth>8", 0, null, listener);
		JMenuItem filter9 =
			createJMenuItemAlt(menu, "Filter out depth>9", 0, null, listener);
		listener.addButtons(
			filterMethodC,
			unfilter,
			filterClass,
	...
	private void createTraceMenu(JMenuBar panel) {
		...
		JMenuItem copy0MenuItem =
			createJMenuItemAlt(menu, "Copy 'this'", 0, null, listener);
		JMenuItem copy1MenuItem =
			createJMenuItemAlt(
				menu,
				"Copy argument 1",
				KeyEvent.VK_1,
		...
		JMenuItem copy5MenuItem =
			createJMenuItemAlt(
				...
				null,
				listener);
		JMenuItem copyrMenuItem =
			createJMenuItemAlt(menu, "Copy return value", 0, null, listener);
		JMenuItem searchMenuItem =
			createJMenuItemCTRL(
				menu,
				"Search",
				KeyEvent.VK_S,
				"Search Trace Pane for string",
				listener);
		JMenuItem rsearchMenuItem =
			createJMenuItemCTRL(
				menu,
				"Reverse Search",
				KeyEvent.VK_R,
				"Reverse search Trace Pane for string",
				listener);
		JMenuItem esearchMenuItem =
			createJMenuItem(
				menu,
				"End Search",
				KeyEvent.VK_ENTER,
				"End Search and revert",
				listener);
		JMenuItem fgetMenuItem =
			createJMenuItemCTRL(menu, "fget", KeyEvent.VK_F, "fget ", listener);
		JMenuItem cdataMenuItem =
			createJMenuItemCTRL(
				menu,
				"cdata",
				KeyEvent.VK_C,
				"cdata ",
				listener);
		JMenuItem fgetStartMenuItem =
			createJMenuItemCTRL(
				menu,
				"Save start pattern",
				0,
				"Save the fget pattern for start",
				listener);
		JMenuItem fgetStopMenuItem =
			createJMenuItemCTRL(
				menu,
				"Save stop pattern",
				0,
				"Save the fget pattern for stop",
				listener);
		JMenuItem countMenuItem =
			createJMenuItemCTRL(
				...
				"Total no. of matches",
				KeyEvent.VK_T,
				"Count matches for current fget/cdata pattern",
				listener);
		JMenuItem fgetSFLMenuItem =
			createJMenuItemCTRL(
				menu,
				"Save start on current line",
				0,
				"Save the fget pattern to start on this line",
				listener);
		JMenuItem fgetQueryMenuItem =
			createJMenuItemCTRL(
				menu,
				"Create FGET query",
				0,
				"Create a query which matches the current traceline",
				listener);
		JMenuItem fgetSLQueryMenuItem =
			createJMenuItemCTRL(
				menu,
				"Create FGET SourceLine query",
				0,
				"Create a query which matches the current SourceLine",
				listener);
		firstLine = new JCheckBoxMenuItem("Go to first line in method");
		firstLine.setState(true);
		menu.add(firstLine);
		/*
		  threadNextCommand = new JCheckBoxMenuItem("Thread previous/next goes to NEXT thread");
		  threadNextCommand.setState(true);
	...
	private void createObjectsMenu(JMenuBar panel) {
		...
		JMenuItem copyClassMenuItem =
			createJMenuItemAlt(
				menu,
				"Add Class",
				KeyEvent.VK_K,
				"Add the Class of the selected object",
				listener);
		...
		JMenuItem expandMenuItem =
			createJMenuItemAlt(
				...
				"Expand",
				KeyEvent.VK_E,
				"Expand/Close the selected object",
				listener);
		JMenuItem selectMenuItem =
			createJMenuItemAlt(
				...
				"Select IV Value",
				KeyEvent.VK_S,
				"Select new value from list of values (Objects Pane)",
				listener);
		JMenuItem localMenuItem =
			createJMenuItemAlt(
				menu,
				"Select Local Value",
				KeyEvent.VK_L,
				"Select new value from list of values (Locals Pane)",
				listener);
		JMenuItem retainMenuItem =
			createJMenuItemAlt(
				menu,
				"Retain Only",
				KeyEvent.VK_O,
				"Retain only the selected IV",
				listener);
		JMenuItem setMenuItem =
			createJMenuItemAlt(
				menu,
				"Assign",
				KeyEvent.VK_A,
				"Assign new value to IV",
				listener);
		...
		JMenuItem abortMenuItem =
			createJMenuItemCTRL(
				...
				"Abort",
				KeyEvent.VK_G,
				"Abort current minibuffer command",
				listener);
		JMenuItem showAllMenuItem =
			createJMenuItemAlt(menu, "ShowAll", 0, "Show all IVs", listener);
		JMenuItem restoreMenuItem =
			createJMenuItemAlt(menu, "Restore", 0, "Restore all IVs", listener);
		JMenuItem addMenuItem =
			createJMenuItemAlt(
				menu,
				"Add Instance Variable",
				KeyEvent.VK_V,
				"Add the selected instance varible value",
				listener);
		JMenuItem hexMenuItem =
			createJMenuItemAlt(
				menu,
				"Special Format",
				KeyEvent.VK_X,
				"Display selected objects in special format",
				listener);
		JMenuItem printMenuItem =
			createJMenuItemAlt(
				...
				"Print",
				0,
				"Print the selected object on terminal",
				listener);
		JMenuItem toStringMenuItem =
			createJMenuItemAlt(
				menu,
				"toString",
				0,
				...
				listener);
		JMenuItem inputObjectMenuItem =
			createJMenuItemAlt(
				menu,
				"Input Object",
				0,
				"Input object from mini-buffer",
				listener);
		listener.addButtons(
			copyClassMenuItem,
			removeMenuItem,
	...
	private void createDebugMenu(JMenuBar panel) {
		...
		JMenuItem recordMenuItem =
			createJMenuItemAlt(menu, "Record", 0, "Record Commands", listener);
		JMenuItem saveMenuItem =
			createJMenuItemAlt(
				menu,
				"Save",
				0,
				"Save Commands to .debuggerCommands",
				listener);
		JMenuItem replayMenuItem =
			createJMenuItemAlt(menu, "Replay", 0, "Replay Commands", listener);
		JMenuItem dumpMenuItem =
			createJMenuItemAlt(
				menu,
				"Dump",
				0,
				"Dump All Data to TTY",
				listener);
		JMenuItem statsMenuItem =
			createJMenuItemAlt(
				menu,
				"Statistics",
				0,
				"Print out statistics to TTY",
				listener);
		JMenuItem revertMenuItem =
			createJMenuItemAlt(
				menu,
				"Revert",
				0,
				"Revert to every time stamp in program",
				listener);
		JMenuItem testMenuItem =
			createJMenuItemAlt(
				...
				"Run a test function",
				listener);
		JMenuItem wideMenuItem =
			createJMenuItemAlt(menu, "Wide", 0, "Show wide strings", listener);
		JMenuItem repeatMenuItem =
			createJMenuItemAlt(
				menu,
				"Repeat",
				KeyEvent.VK_SPACE,
				"Repeat last button command",
				listener);
		listener.addButtons(
			recordMenuItem,
			saveMenuItem,
	...
	private JMenuItem createJMenuItemAlt(
		JMenu menu,
		String text,
		int key,
		String tooltip,
	...
	private JMenuItem createJMenuItemCTRL(
		JMenu menu,
		String text,
		int key,
		String tooltip,
	...
	private void createCodeMenu(JMenuBar panel) {
		...
		JMenuItem gotoStackJMenuItem =
			createJMenuItemAlt(
				menu,
				"Goto Stack Frame",
				0,
				"Goto selected stack frame",
				listener);
	...
	private void createFileMenu(JMenuBar panel) {
		...
		JMenuItem openJMenuItem =
			createJMenuItemAlt(menu, "Open", 0, "Open File", listener);
		JMenuItem exitJMenuItem =
			createJMenuItemAlt(menu, "Exit", 0, "Open File", listener);
		JMenuItem addJMenuItem =
			createJMenuItemCTRL(
				menu,
				"Add Mark",
				KeyEvent.VK_SPACE,
				"Add a mark to the ring",
				listener);
		JMenuItem previousJMenuItem =
			createJMenuItemCTRL(
				menu,
				"Previous Mark",
				KeyEvent.VK_X,
				"Cycle one mark in the ring",
				listener);
	...
	private void createRunMenu(JMenuBar panel) {
		...
		JMenuItem restartJMenuItem =
			createJMenuItemAlt(
				...
				listener);
		JMenuItem clearJMenuItem =
			createJMenuItemAlt(menu, "Clear", 0, "Clear everything.", listener);
		JMenuItem startJMenuItem =
			createJMenuItemAlt(
				menu,
				"Start Recording on Output",
				0,
				"Start Recording on Selected TTY Output",
				listener);
		JMenuItem stopJMenuItem =
			createJMenuItemAlt(
				menu,
				"Stop Recording on Output",
				0,
				"Stop Recording on Selected TTY Output",
				listener);
		JMenuItem startLJMenuItem =
			createJMenuItemAlt(
				menu,
				"Start Recording on Line",
				0,
				"Start Recording on Selected Line",
				listener);
		JMenuItem stopLJMenuItem =
			createJMenuItemAlt(
				menu,
				"Stop Recording on Line",
				0,
				"Stop Recording on Selected Line",
				listener);
		JMenuItem callJMenuItem =
			createJMenuItemCTRL(
				menu,
				"Evaluate Expression",
				KeyEvent.VK_E,
				"Example: <MyObj_1>.copy(<MyObj_2>, 1)",
				listener);
		JMenuItem switchJMenuItem =
			createJMenuItemCTRL(
				menu,
				"Switch Timelines",
				KeyEvent.VK_O,
				...
				listener);
		JMenuItem clearSSJMenuItem =
			createJMenuItemCTRL(
				menu,
				"Clear Start/Stop",
				0,
				"Clear All Start/Stop Request",
				listener);
		listener.addButtons(
			restartJMenuItem,
			clearJMenuItem,
