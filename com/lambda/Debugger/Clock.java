...
public class Clock implements Runnable {
    ...
    public static int getTS(int i) {return timeStamps[i];}
    public static int size() {return index;}
	...
	while (true) {
	    ...
	    if (D.PAUSE_PROGRAM) continue;
	    now = TimeStamp.eott();
	    if (timeStamps[index-1] == now) continue;
	    long clockTime = System.currentTimeMillis();
	    if (clockTimes[index-1] == clockTime) continue;
	...
	if (index < 2) return "NoClock";
	endTime = clockTimes[index-2];
	String s = " Clock[" + formatTime(endTime) + "] " + formatTime(findTime(time));
	return s;
    }
	...
	for (int i = index-1; i > -1; i--) {
	    if (timeStamps[i] < time) return clockTimes[i];
	}
	return 0;
    }
	...
	for (int i = 1; i < index; i++) {
	    ...
	    int ave=0;
	    if (t/10 != 0)  ave = s/((int)t/10);
	    System.out.println(""+i+
			       "\t"+formatTime(clockTimes[i])+
			       "\t"+formatTime(t)+
			       "\t"+timeStamps[i]+
			       "\t"+s+
			       "\t"+ave);
